...
export default function AdminPanel() {
	...
	useEffect(() => {
		...
		if (!isLoggedIn) { router.push('/agent/login'); return; }
	...
	return (
		<div className="min-h-screen bg-[#edf3f6] dark:bg-zinc-950 font-sans text-slate-800 dark:text-zinc-100 flex flex-col">
			...
			<header className="bg-[#fff7cd] dark:bg-zinc-900 border-b border-[#ebd7a3] dark:border-zinc-800 px-6 py-4 flex items-center gap-4 shrink-0">
				<button onClick={() => router.push('/agent/dashboard')} className="w-8 h-8 rounded-full bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 flex items-center justify-center hover:bg-slate-50 transition-all cursor-pointer shadow-sm">
					<ArrowLeftIcon className="w-3.5 h-3.5 text-slate-600 dark:text-zinc-300" />
				</button>
				<div>
					<h1 className="text-sm font-bold tracking-wide uppercase">SONA SCALE UWA — Admin Panel</h1>
